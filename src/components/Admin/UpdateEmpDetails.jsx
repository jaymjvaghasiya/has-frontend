...
export const UpdateEmpDetails = () => {
  ...
  const { register, handleSubmit, formState: { errors } } = useForm({
    defaultValues: async () => {
      ...
      return {
        ...
        titleName: res.data.data?.titleName,
        ...
        activeInd: res.data.data?.activeInd,
        dateOfJoining: res.data.data?.dateOfJoining,
        aadharCardPath: res.data.data?.aadharCardPath
      }
    }
  });
  ...
  const submitHandler = async (d) => {
    ...
    try {
      let res = await axios.put(`http://localhost:3001/employee/${id}`, formData);
      if (res.status === 200) {
